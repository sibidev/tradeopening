...
#7 and 21 days moving average, exponential moving average, momentum, Bollinger bands, MACD.
...
equities <-
  equities %>%
  group_by( Company ) %>%
  mutate(  ma7 = TTR::SMA(prev_close, n = 7)  )

equities <-
  equities %>%
  group_by( Company ) %>%
  mutate(  ma21 = TTR::SMA(prev_close, n = 21)  )
...
lda_fit=lda(Direction ~ prev_return + prev_return_2 + prev_return_3 + prev_return_4+
              prev_return_5 + opening_perf + ma7 + ma21
            ,data=equities_train)
...
xgbTree_fit <- caret::train(Direction ~  prev_return_4+
                              prev_return_5 + opening_perf + ma7  + ma21,
                            data= equities_train,
...
#VarImp after XGBoost:
Overall
opening_perf  100.00000
prev_return_5  16.17954
prev_return_4  12.75574
ma200          12.63285
ma50           11.20646
prev_return    10.89127
prev_return_2   6.15908
prev_return_3   0.00000
